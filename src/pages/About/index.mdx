---
title: Mrunmayee Bhagwat
---
import useBaseUrl from '@docusaurus/useBaseUrl';


<img src={useBaseUrl('/img/12786.jpg')} alt="Example banner"
  width="30%"
/>


export const Highlight = ({children, color}) => (
  <span
    style={{
      backgroundColor: color,
      borderRadius: '2px',
      color: '#17202A',
      padding: '0.2rem',
    }}>
    {children}
  </span>
);


# Welcome to my portfolio website!

Passionate about building impactful applications, I am currently a Software Engineer with over 4 years of experience and a MS CS graduate student. I thrive on contributing to cutting-edge projects, honing my skills in Full Stack Development, and staying at the forefront of industry trends.
<br/>

### <Highlight color="#039dfc">Profile Summary</Highlight>

- Demonstrated prowess in full-stack development using technologies like React, Spring Boot, and Node.js.
- Experience in creating Test strategies and Test plans for large scale software projects in diverse domains such as Insurance , Retail and Corporate Banking.
- Expertise in designing, creating and maintaining Test automation frameworks supporting Web and Mobile(Android and iOS) platforms which can be easily integrated in continuous integration environment.
- Fluent in programming languages like Java, JavaScript and Automation Tools like Selenium , Webdriver IO, Appium .
- Expertise in Agile methodologies for development and testing throughout the software development life cycle of the project, played part of multiple scrum teams with client interaction.
- Good debugging, analytical and problem solving skills.

<br/>

### <Highlight color="#039dfc">Technical Skills</Highlight>


| Category | Skills |
| ---- | ---- |
| `Programming Languages` | Java, Javascript, C, C++, PHP, Ruby(Basics), HTML5 , CSS |
| `Automation Tools` | Selenium , Webdriver IO , Appium |
| `Tools` | JIRA, Confluence, Jmeter, Postman, Burpsuite |
| `IDE` | Eclipse, IntelliJ, VS Code |
| `Database` | MYSQL, MongoDB, Cassandra |
| `Data Exchange Format` | XML, JSON, YAML |
| `Operating Systems` | Windows7 and 10, iOS |

<br/>

### <Highlight color="#039dfc">Work Experience</Highlight>

<br/>

##### <Highlight color="#039dfc">1)Larsen and Toubro Infotech , Pune   ( 24/07/2018 – 12/03/2021)</Highlight>
- **Role** : Software Engineer
- **Application Domains**: Banking & Finance , Insurance
- **Responsibilities**: <br/>
► Automated Validation of Delinquency Letters in Insurance Domain. Implemented end-to-end automation using Spring Boot, Java, and RedSix PDF Comparison Library. Achieved a 95% pass rate, eliminating human errors and enhancing accuracy. Reduced manual efforts, improving eﬀiciency and expediting the delinquency workflow <br/>
► Automated REST API testing using Karate. Improved test eﬀiciency by parallelizing API tests, reducing the overall execution time by 50% <br/>

<br/>

##### <Highlight color="#039dfc">2)ThoughtWorks , India   ( 12/03/2021 – Present)</Highlight>
- **Role** : Consultant
- **Application Domains**: Banking & Finance
- **Responsibilities**: <br/>
► Led the development and management of 20+ automation scripts using WebdriverIO and Appium frameworks, resulting in a 30% reduction in manual testing efforts. Orchestrated parallel execution strategies, leading to a 40% decrease in test suite execution time <br/>
► Devised and executed a mitigation strategy for build failures, resulting in a 20% decrease in downtime. Additionally, served as the team representative for Root Cause Analysis (RCA) in build failure analysis calls <br/>
► Enhancing and maintaining automation scripts for smoke and regression testing using automation tools like Webdriver and Appium  <br/>

<br/>

##### <Highlight color="#039dfc">3)Arena Investors, LP , New York City   ( 22/05/2023 – 9/12/2023)</Highlight>
- **Role** : Software Engineer Intern
- **Application Domains**: Private Credit
- **Responsibilities**: <br/>
► Spearheaded the design and development of a Slack message processing system, handling 10,000+ messages across multiple channels, translating them into the database, and reducing email reliance by 70%. <br/>
► Developed dynamic data tables using jQuery DataTables, enhancing data visualization and user experience.
► Integrated DataTables with backend APIs for real-time data display, optimizing performance, reducing page load times by 20%, and boosting user engagement by 35%. <br/>

<br/>

### <Highlight color="#039dfc"> Education </Highlight>
► Bachelor of Engineering ( B.E. ) (Information Technology)  3.81/4.00 (2014-2018)
[P.V.G’s College of Engineering and Technology][college] ,[Savitribai Phule Pune University, Pune][university]

<br/>

► Master of Science ( MS ) (Computer Science)  3.97/4.00 (2022-2024)
[George Mason University][gmu], Virginia, USA

<br/>

► HSC , Science (2012-2014)
[Muktangan English School & Jr. College][jouniorcollege] ,[Maharashtra State Board][msbshse]

<br/>

► SSC (2002-2012)
[D.E.S Secondary School][school] ,[Maharashtra State Board][msbshse]

<br/>

### <Highlight color="#039dfc"> Papers Presented </Highlight>
► Published a paper entitled “Melanoma Prediction using Machine Learning” in IEEE journal and presented the same in [Second International Conference on Electronics, Communication and Aerospace Technology (ICECA 2018)][IEEE], at the RVS Technical Campus, Coimbatore, Tamilnadu, India during 29-31st March , 2018.

<br/>

### <Highlight color="#039dfc"> Achievements & Extra-curricular Involvements </Highlight>
	•	Pursuing Graduate Degree Course in Classical Music from ‘Gandharva Mahvidyalaya, Pune’. Cleared 6 exams with Distinction.
	•	Participated in a 10km Marathon organized in Balewadi,  Pune. Earned a finisher medal.
	•	Silver Medal and certificate of merit for securing 2nd Rank in final year in IT dept.


[project]: /projects
[blog]: /blog/
[org]: https://www.thoughtworks.com/
[college]: https://www.pvgcoet.ac.in/
[gmu]: https://www.gmu.edu/
[university]: http://www.unipune.ac.in/
[IEEE]: http://toc.proceedings.com/41134webtoc.pdf
[Music]: https://abgmvm.org/
[jouniorcollege]: http://www.muktanganenglishschool-jrcollege.in/
[msbshse]: https://www.mahahsscboard.in/
[school]:https://www.despune.org/courses/des-secondary-school/
